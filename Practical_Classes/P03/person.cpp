...
class person
{
  ...
  public:
    ...
    void set_phoneNumber(const int number);
    const char *get_name(void);
    const char *get_address(void);
    const int get_phoneNumber(void);
};
...
void person::set_phoneNumber(const int number)
{
  phone_number = number;
}
...
inline const int person::get_phoneNumber(void)
{
  return phone_number;
}
...
int main(void)
{
  person p = person("John Doe","no address", 912345678); // create a new object and then copy it to p
